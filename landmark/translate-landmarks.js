 ...
 * 번역이 필요한 필드만 추출
 */
function extractFieldsForTranslation(koData, existingData) {
  ...
  for (const field of CONFIG.TRANSLATABLE_FIELDS) {
    ...
    // 문자열인 경우: 한국어가 포함되어 있으면 번역 필요
    if (typeof existingValue === 'string') {
      if (containsKorean(existingValue)) {
        fieldsToTranslate[field] = koData[field];
      }
    } 
    // 배열인 경우: 하나라도 한국어가 있으면 번역 필요
    else if (Array.isArray(existingValue)) {
      const hasKorean = existingValue.some(item => 
        typeof item === 'string' && containsKorean(item)
      );
      if (hasKorean) {
        fieldsToTranslate[field] = koData[field];
      }
    }
    // 객체인 경우 (score_reasons): 재귀적으로 확인
...
function parseJSONResponse(responseText) {
  ...
  // 세 번째 시도: 문자열 내 이스케이프 안 된 따옴표 처리
  try {
    // 배열 내 문자열에서 이스케이프 안 된 따옴표를 수정
    // "text "quoted" more" -> "text \"quoted\" more"
    let fixed = jsonStr.replace(/"([^"]*)"([^",:}\]\n]+)"([^"]*)"/g, '"$1\\"$2\\"$3"');
    return JSON.parse(fixed);
  } catch (e) {
    // 다음 복구 시도
  ...
  // 네 번째 시도: 중국어/일본어 따옴표를 일반 따옴표로 변환
  try {
    let fixed = jsonStr
      .replace(/"/g, '"')  // 중국어 왼쪽 따옴표
      .replace(/"/g, '"')  // 중국어 오른쪽 따옴표
      .replace(/「/g, '"')  // 일본어 왼쪽 따옴표
      .replace(/」/g, '"')  // 일본어 오른쪽 따옴표
      .replace(/『/g, '"')
      .replace(/』/g, '"')
      .replace(/"/g, '\\"'); // 문자열 내 따옴표 이스케이프
...
async function translateLandmark(koData, existingData, targetLang, model) {
  ...
  // 번역이 필요한 필드만 추출
  const fieldsToTranslate = extractFieldsForTranslation(koData, existingData);
  ...
  // 기존 데이터에 번역된 필드 병합
  let result = existingData ? { ...existingData } : { ...koData };
  ...
  if (scoreReasons && Object.keys(scoreReasons).length > 0) {
    result.score_reasons = result.score_reasons || {};
    ...
    for (let i = 0; i < scoreKeys.length; i++) {
      ...
      try {
        ...
        if (translatedFields.score_reasons && translatedFields.score_reasons[scoreKey]) {
          result.score_reasons[scoreKey] = translatedFields.score_reasons[scoreKey];
        }
        ...
        // 기존 데이터 유지
        if (existingData?.score_reasons?.[scoreKey]) {
          result.score_reasons[scoreKey] = existingData.score_reasons[scoreKey];
        } else if (koData?.score_reasons?.[scoreKey]) {
          result.score_reasons[scoreKey] = koData.score_reasons[scoreKey];
        }
      }
    }
  }
  ...
  for (let i = 0; i < needsTranslation.length; i++) {
    ...
    try {
      ...
      if (!result.skipped && result.usage) {
        ...
        console.log(`    ✓ 저장 완료 (${translatedCount}/${needsTranslation.length}) | 비용: ${formatCost(cost)}\n`);
