/**
 * 한중 번역 검수 및 선택적 재번역 스크립트
 * 
 * 1단계: 한국어 원본과 중국어 번역을 비교하여 문제 탐지
...
const ZH_DB_PATH = path.join(__dirname, "..", "db_zh.js");
const OUTPUT_PATH = path.join(__dirname, "..", "db_zh.js");
const REPORT_PATH = path.join(__dirname, "review-report-zh.json");
 ...
 * 번역 품질 검수 (배치)
 */
async function reviewTranslationBatch(koItems, zhItems) {
  const pairs = koItems.map((ko, i) => ({
    id: ko.id,
    name_ko: ko.name,
    name_zh: zhItems[i].name,
    summary_ko: ko.summary,
    summary_zh: zhItems[i].summary,
    description_ko: ko.description,
    description_zh: zhItems[i].description,
    tips_ko: ko.tips,
    tips_zh: zhItems[i].tips
  }));

  const systemPrompt = `You are an expert translation quality reviewer for Korean to Simplified Chinese tourism content.

Your task is to compare Korean originals with Chinese translations and identify ANY issues:

1. **Mistranslation**: Meaning changed or wrong
2. **Omission**: Important information missing
3. **Awkward phrasing**: Unnatural Chinese that sounds like translation
...
Be strict - we want A+ quality translations suitable for professional tourism guides.`;
  ...
  const userPrompt = `Review these Korean→Chinese translations:
 ...
 * 단일 항목 재번역 (고급 모델)
 */
async function retranslateItem(koItem, issues, retryCount = 0) {
  const userPrompt = `Translate Korean tourism content to Simplified Chinese.

INPUT:
name: ${koItem.name}
summary: ${koItem.summary}
description: ${koItem.description}
tips: ${koItem.tips.join(' | ')}

ISSUES: ${issues.map(i => i.problem).join('; ')}

OUTPUT only this JSON format, nothing else:
{"name":"名称","summary":"简介","description":"详细描述","tips":["提示1","提示2","提示3","提示4"]}`;
  ...
  try {
    ...
    if (startIdx === -1 || endIdx === -1) {
      console.error(`    JSON 없음`);
      if (retryCount < 2) {
        console.log(`    재시도 ${retryCount + 1}/2...`);
        await new Promise(r => setTimeout(r, 1000));
        return retranslateItem(koItem, issues, retryCount + 1);
      }
      return null;
    }
    ...
    try {
      ...
      // description 내부의 따옴표가 문제일 수 있음
      // 필드별로 추출 시도
      try {
        const nameMatch = jsonStr.match(/"name"\s*:\s*"([^"]+)"/);
        const summaryMatch = jsonStr.match(/"summary"\s*:\s*"([^"]+)"/);
        const descMatch = jsonStr.match(/"description"\s*:\s*"([\s\S]+?)"\s*,\s*"tips"/);
        const tipsMatch = jsonStr.match(/"tips"\s*:\s*\[([\s\S]+?)\]/);
        
        if (nameMatch && summaryMatch && descMatch && tipsMatch) {
          const tips = tipsMatch[1].match(/"([^"]+)"/g)?.map(t => t.replace(/"/g, '')) || [];
          return {
            name: nameMatch[1],
      ...
      console.error(`    파싱 실패: ${e.message.substring(0, 50)}`);
 ...
 */
async function reviewScoreReasons(koItem, zhItem) {
  const systemPrompt = `You are reviewing Korean→Simplified Chinese translations of tourism rating explanations.

Compare the Korean original with Chinese translation for each category.
...
Chinese score_reasons:
${JSON.stringify(zhItem.score_reasons, null, 2)}
...
async function main() {
  ...
  const zhData = loadDBFile(ZH_DB_PATH);
  console.log(`  ✓ 한국어: ${koData.length}개`);
  console.log(`  ✓ 중국어: ${zhData.length}개\n`);

  // ID로 매핑
  const zhMap = new Map(zhData.map(item => [item.id, item]));
  ...
  for (let i = 0; i < koData.length; i += CONFIG.BATCH_SIZE) {
    ...
    const zhBatch = batch.map(ko => zhMap.get(ko.id));
    
    process.stdout.write(`  검수 중: ${i + 1}-${Math.min(i + CONFIG.BATCH_SIZE, koData.length)}/${koData.length}...`);
    
    const result = await reviewTranslationBatch(batch, zhBatch);
...
async function fixIssues() {
  ...
  if (!fs.existsSync(REPORT_PATH)) {
    console.log("❌ 먼저 검수를 실행하세요: node review-translation-zh.js");
    return;
  }
  ...
  const koData = loadDBFile(KO_DB_PATH);
  let zhData = loadDBFile(ZH_DB_PATH);
  ...
  for (let i = 0; i < report.issues.length; i++) {
    ...
    const zhIndex = zhData.findIndex(e => e.id === issue.id);
    
    if (!koItem || zhIndex === -1) continue;
    ...
    if (newTranslation) {
      // 기존 데이터 업데이트
      if (newTranslation.name) zhData[zhIndex].name = newTranslation.name;
      if (newTranslation.summary) zhData[zhIndex].summary = newTranslation.summary;
      if (newTranslation.description) zhData[zhIndex].description = newTranslation.description;
      if (newTranslation.tips) zhData[zhIndex].tips = newTranslation.tips;
      ...
      // DB도 즉시 저장
      saveDBFile(OUTPUT_PATH, zhData, "zh");
    } else {
      console.log(`    ❌ 실패`);
      failedCount++;
    }
  ...
  if (report.issues.length > 0) {
    console.log(`\n⚠️  ${report.issues.length}개 남음 - 다시 시도: node review-translation-zh.js --fix`);
  }
}
